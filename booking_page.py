...
# Page setup
st.set_page_config(layout="wide", page_icon=":health_worker:")
st.markdown(
...
time_slot = ["9:00 AM", "10:00 AM", "11:00 AM", "12:00 PM", "1:00 PM",\
    ...
    for idx in range(len(available_days)):
        ...
        with col[col_idx]:
            ...
            if available_days[idx] in unavailable_days:
                st.button(available_days[idx], disabled=True, key=available_days[idx])
            else:
...
with booking_column:
    ...
    if st.button('Đặt hẹn'):
        if name == "":
            ...
            st.success(f"Đặt lịch hẹn thành công với {doctor_name}."
